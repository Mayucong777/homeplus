...
public class AppointmentServiceImpl implements AppointmentService {
    ...
    @Override
    public boolean insertAppointment(Appointment appointment, String username) throws AppointFailedException, UserNoLoginException {
        Integer cmID = appMapper.getCustomerIDByUsername(username);
        Customer customer = personMapper.selectCustomer(username);
        if (cmID == null) {
            throw new UserNoLoginException("用户未登录！");
        }else if(customer.getCmStatus() !=1){
            throw new AppointFailedException("预约失败，请先完成用户认证！");
        }
        appointment.setCmID(cmID);
        boolean result = appMapper.insertAppointment(appointment);
        if (result) {
            return true;
        } else {
    ...
    public Appointment selectAppointByID(int id) throws UserNoLoginException {
        ...
        if(app.getHkID()!= NULL){
            String hkNickname = userMapper.selectHKNicknameByHkID(app.getHkID());
            app.setHkNickname(hkNickname);
        }
        // 将申请列表的家政人员的昵称放到实体类中
