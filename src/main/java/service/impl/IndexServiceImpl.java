...
public class IndexServiceImpl implements IndexService {
    ...
    @Override
    public List<Type> getAllTypeDetail() {
        return indexMapper.getAllTypeDetail();
    }

    @Override
    public List<HouseKeeper> getHighScoreHK() {
        //得到好评最多的HKID
        List<Integer> hkIDList = indexMapper.getHighScoreHKID();
        List<HouseKeeper> list = new ArrayList<>();
        for (int i : hkIDList) {
            HouseKeeper houseKeeper =  hkPersonMapper.getHousekeeperByID(i);
            list.add(houseKeeper);
        ...
        for (HouseKeeper hk : list) {
            String companyName = indexMapper.getCompanyNameByID(hk.getCompanyID());
            hk.setCompanyName(companyName);
        }
        return list;
    }
