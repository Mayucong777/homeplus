    ...
    $(".search-btn").click(function () {
        ...
        $.ajax ({
            ...
            "success": function (json) {
                if (json.state == 200) {
                    var id = json.data;
                    window.location.href = "http://localhost:8080/search-result.jsp?typeid=" + id;
                }
            }
        })
    })
	...
	$.ajax({
		...
		"success": function (json) {
			...
			for (var i = 0; i < list.length; i ++) {
                ...
                /*var type_option = "<option class=\"list-group-item app-type\" id='#{id}'>#{type_name}</option>";
                $("#app_type").append(type_option);*/
			}
        }
	})
}
            ...
            if (json.state == 200) {
                ...
                for (var i = 0; i < list.length; i ++) {
                    var optionData = "<li class=\"list-group-item\">\n" +
                        "<a href=\"search-result.jsp?typeid=#{typeid}\">#{small_type_name}</a>\n" +
                        "</li>";
...
function loadAppType() {
    var url = "index/getAllTypeDetail";
    $.ajax({
        "url" : url,
        "type":"post",
        "success": function (json) {
            if(json.state==200){
                var list = json.data;
                for(var i=0;i<list.length;i++){
                    var appTypeDetailData = "<option>#{small_type_name}</option>";
                    appTypeDetailData = appTypeDetailData.replace("#{small_type_name}",list[i].name);
                    $("#app_type").append(appTypeDetailData);
                }
            }
        }
    })
        ...
        "success": function (json) {
            if (json.state == 200) {
                ...
                for (var i = 0; i < list.length; i ++) {
                    ...
                    optionData = optionData.replace("#{user_detail}", "housekeeper.jsp?id=" + list[i].id );
        ...
        "success": function (json) {
            ...
            for (var i = 0; i < list.length; i ++) {
                var optionData=
                    ...
                    "</div>\n" +
                    "<div class=\"col-md-3\">\n" +
                    "<a href=\"housekeeper.jsp?id=#{hk_id}\" class=\"btn btn-primary go-single\">查看详情</a>\n" +
                    "</div>\n" +
                    "</div>";
